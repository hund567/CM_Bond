...
def name2df_trans_bond(filename):
    ...
    #出现乱码问题则用下面这个encoding
    #data = pd.read_csv(path_general+filename, encoding='gb18030', index_col=[0])
    ...
    # 提取债券类型和发行人类型，将无债券类型或发行人的设定为“None”
    #用于生成待查询的code名称集合
    def execute(sql):
    ...
    #将当日债券代码上传生成暂时表Temporary用于比对
    latest_code = data.iloc[:,:2]
    insert_db(latest_code,'Temporary','notacsv')

    #与字典比对并生成结果
    sql=f"CREATE VIEW {db_name}.codematch AS     SELECT code,issuer,windtype,issue_listance FROM {db_name}.Dict_for_transaction_bond     JOIN {db_name}.Temporary USING (code)"
    execute(sql)

    #读取匹配结果，筛选出原列表中不在字典中的内容
    engine =  create_engine('mysql+pymysql://' + user + ":" + password + "@" + db_ip + ":" + str(port) + "/" + db_name,
                           encoding='utf-8')
    sql=f'select * from {db_name}.codematch;'
    latest_code_with_info=pd.read_sql(sql,engine,index_col='code')
    latest_code_without_info = set(latest_code.code).difference(set(latest_code_with_info.index))
    execute(f'drop table {db_name}.Temporary; ')

    #将code名前+'0'再次匹配,重复以上操作
    adj_code='0'+latest_code.loc[latest_code['code'].isin(latest_code_without_info)]
    insert_db(adj_code,'Temporary','notacsv')
    #比对
    sql=f"CREATE VIEW {db_name}.codematch AS     SELECT code,issuer,windtype,issue_listance FROM {db_name}.Dict_for_transaction_bond     JOIN {db_name}.Temporary USING (code)"
    execute(sql)
    #筛选
    sql=f'select * from {db_name}.codematch;'
    adj_latest_code_with_info=pd.read_sql(sql,engine,index_col='code')
    #latest_code_without_info = set(adj_code.code).difference(set(adj_latest_code_with_info.index))
    execute(f'drop table {db_name}.Temporary; ')
    execute(f'drop view {db_name}.codematch; ')
    #对原数据的code遍历，从with_info中获取券种和发行人，不在上述两个表中的，从wind获取
    dic_append=pd.DataFrame({'code':[],'name':[],'issuer':[],'windtype':[],'issue_listance':[]})
    dic_append.set_index(['code'],inplace=True)
    dic_name='Dict_for_transaction_bond'
    data['issuer']=None
    data['windtype']=None
    data['issue_listance']=None
    data['term_cat']=None
    for j in range(len(data)):

        if str(data.iloc[j,0]) in latest_code_with_info.index :
            data.iloc[j,21]=latest_code_with_info.loc[str(data.iloc[j,0]),'issuer']
            data.iloc[j,22]=latest_code_with_info.loc[str(data.iloc[j,0]),'windtype']
            data.iloc[j,23]=latest_code_with_info.loc[str(data.iloc[j,0]),'issue_listance']
            
        elif '0'+str(data.iloc[j,0]) in adj_latest_code_with_info.index:
            data.iloc[j,0]='0'+data.iloc[j,0]
            data.iloc[j,21]=adj_latest_code_with_info.loc[str(data.iloc[j,0]),'issuer']
            data.iloc[j,22]=adj_latest_code_with_info.loc[str(data.iloc[j,0]),'windtype']
            data.iloc[j,23]=adj_latest_code_with_info.loc[str(data.iloc[j,0]),'issue_listance']

        else:
            source=w.wss(str(data.iloc[j,0]), "sec_name,issuerupdated,windl2type,issue_listance").Data
            if source[0][0]==None:
                data.iloc[j,0]='0'+data.iloc[j,0]
                source=w.wss(str(data.iloc[j,0]), "sec_name,issuerupdated,windl2type,issue_listance").Data
            print(source)
            dic_append.loc[str(data.iloc[j,0]),'name']=source[0][0]
            dic_append.loc[str(data.iloc[j,0]),'issuer']=source[1][0]
            dic_append.loc[str(data.iloc[j,0]),'windtype']=source[2][0]
            dic_append.loc[str(data.iloc[j,0]),'issue_listance']=source[3][0]
            data.iloc[j,21]=dic_append.loc[str(data.iloc[j,0]),'issuer']
            data.iloc[j,22]=dic_append.loc[str(data.iloc[j,0]),'windtype']
            data.iloc[j,23]=dic_append.loc[str(data.iloc[j,0]),'issue_listance']
    #根据maturi的值给term_cat赋值
    def maturity2term(x):
        keyterm=[0,1,3,5,7,10]
        for i in range(len(keyterm)):
            if x>keyterm[i]:
                continue
            return str(keyterm[i-1])+'-'+str(keyterm[i])
        return '10+'
    data.term_cat=data.maturity.apply(lambda x: maturity2term(x))
    insert_db(dic_append,dic_name,'bond_dic')
...
if __name__ == '__main__':
    ...
    port = 10059
    #文件路径
    path_general = "C:\\Users\\Joyce Lin\\Desktop\\dir\\input\\"
    #数据库创建判断
    conn = pymysql.connect(user=user, password=password, host=db_ip, port=port,charset='utf8')
    cur = conn.cursor()
    try:
