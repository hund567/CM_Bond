...
def name2df_trans_bond(filename):
    ...
    # 提取债券类型和发行人类型，将无债券类型或发行人的设定为“其他”
    
    engine =  create_engine('mysql+pymysql://' + user + ":" + password + "@" + db_ip + ":" + str(port) + "/" + db_name,
                       encoding='utf-8')
    dic_name='Dict_for_transaction_bond'
    sql=f'select * from {db_name}.{dic_name};'
    dic=pd.read_sql(sql,engine,index_col='code')
    indexlist=[]
    data['issuer']=None
    data['windtype']=None
    for j in range(len(data)):
        if str(data.iloc[j,0]) in dic.index.to_list():
            pass
        else:
            source=w.wss(str(data.iloc[j,0]), "sec_name,issuerupdated,windl2type").Data
            if source[0][0]==None:
                data.iloc[j,0]='0'+data.iloc[j,0]
                source=w.wss(str(data.iloc[j,0]), "sec_name,issuerupdated,windl2type").Data
            
            indexlist.append(str(data.iloc[j,0]))
            dic.loc[str(data.iloc[j,0]),'name']=source[0][0]
            dic.loc[str(data.iloc[j,0]),'issuer']=source[1][0]
            dic.loc[str(data.iloc[j,0]),'windtype']=source[2][0]

        data.iloc[j,21]=dic.loc[str(data.iloc[j,0]),'issuer']
        data.iloc[j,22]=dic.loc[str(data.iloc[j,0]),'windtype']

    dic_append=dic.loc[indexlist]
...
if __name__ == '__main__':
    ...
    # path_general = "C:\\Users\\Joyce Lin\\Desktop\\input\\"
    #数据库创建判断
